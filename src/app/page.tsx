  ...
  return (
    ...
    >
      ...
      <div className="relative h-2 bg-claude-surface rounded-full overflow-hidden">
        <div
          className={`absolute inset-y-0 left-0 rounded-full progress-animate ${getBarColor(utilization)} ${getBarGlow(utilization)} transition-all`}
...
function RateLimitCard({
  ...
  return (
    <div
      className="fade-in-up bg-claude-surface/60 rounded-xl p-4 border border-claude-border/50 hover:border-claude-border transition-colors"
      style={{ animationDelay: `${delay}ms` }}
    >
...
export default async function DashboardPage() {
  ...
  let rateLimits: RateLimitData | null = null;
  let error: string | null = null;

  try {
    [usage, rateLimits] = await Promise.all([getUsage(), getRateLimits()]);
  } catch (e) {
    error = e instanceof Error ? e.message : "Failed to fetch data";
  }

  if (error || !usage || !rateLimits) {
    return <ErrorState message={error || "No data returned"} />;
  }

  // Group rate limiters by model
    ...
    <div className="min-h-screen bg-claude-bg">
      ...
      <main className="relative max-w-2xl mx-auto px-6 py-12">
        ...
        <section className="mb-10">
          <h2
            className="fade-in-up text-[13px] font-medium uppercase tracking-wider text-claude-text-dim mb-5"
        ...
        {/* Divider */}
        <div
          className="fade-in-up h-px bg-gradient-to-r from-transparent via-claude-border to-transparent mb-10"
        ...
        {/* Footer */}
        <footer
          className="fade-in-up mt-12 text-center text-[12px] text-claude-text-dim"
        ...
        >
          ...
          <p className="mt-1">Refresh the page to update · Not an official API</p>
        </footer>
      </main>
    </div>
  );
